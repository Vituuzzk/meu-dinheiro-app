(function(){
    ...
    function renderizarDashboard() {
        ...
        contasNormais.forEach(c => {
            ...
            div.innerHTML = `<div class="conta-info"><div class="conta-icone">💰</div><div class="conta-detalhes"><div class="nome">${c.nome}</div><div class="subtitulo">${c.incluirNoTotal ? 'Incluída' : 'Não incluída'}</div></div></div><div class="conta-saldo" style="color: ${saldo < 0 ? '#dc2626' : '#1f2937'}">${formatarMoeda(saldo)}</div>`;
    ...
    function renderizarListaContasModal() {
        ...
        contas.forEach(c => {
            ...
            li.innerHTML = `<div><strong>${c.nome}</strong> (${c.tipo==='credito'?'💳':'💰'})<br><small>${formatarMoeda(saldo)}</small></div><button data-id="${c.id}" style="width:auto; background:#ef4444;">🗑️</button>`;
